const getPageTitle = async (page) => {
    return await page.evaluate(() => {
        return 'H1: ' + document.querySelector('.page-title h1').innerHTML.replace(/^\s+|\s+$/g, '')
    });
};

const getMeinInf = async (page) => {
    const meinInf = await page.evaluate(() => {
        ...
        for (let i = 0; i < parentDiv.length; i++) {
            ...
            if (child.tagName === 'P' || child.tagName === 'H2') {
                ...
                postContent.push(`${child.children[0].children[0].tagName}: ${child.children[0].children[0].src}`);
            }
        }
...
module.exports = {getPageTitle, getMeinInf};
